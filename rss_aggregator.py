...
recent_entries = [
    ...
    if datetime.datetime.strptime(entry.published, "%a, %d %b %Y %H:%M:%S %Z").replace(tzinfo=datetime.timezone.utc) >= time_threshold
]
...
last_build_date.text = datetime.datetime.now(
...
# Output RSS feed entry count (only once)
